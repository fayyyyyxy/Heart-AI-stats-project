...
df = pd.read_csv('/Users/student/Desktop/ANON36_MACE_EKG_output.csv')
...
fpr1, tpr1, roc_auc1, label1 = train_and_get(df, X_columns, y_column, 'MACE_ML', categorical_columns=categorical_columns)
fpr2, tpr2, roc_auc2, label2 = train_and_get(df, ['Total CAC Score'], y_column, 'MACE_CAC_Total')
fpr3, tpr3, roc_auc3, label3 = train_and_get(df,['ASCVD PCE Risk'], y_column, 'MACE_PCE')
fpr4, tpr4, roc_auc4, label4 = train_and_get(df,['PREVENT ASCVD Risk'], y_column, 'MACE_PREVENT_ASCVD')
fpr5, tpr5, roc_auc5, label5 = train_and_get(df,['PREVENT CVD Risk'], y_column, 'MACE_PREVENT_CVD')
